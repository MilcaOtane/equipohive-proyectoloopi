...
export default function Challenges() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-accent/10 p-8">
      <div className="container max-w-6xl mx-auto">
        ...
        <Card className="p-8 mb-8 bg-gradient-to-br from-card to-primary/5 border-primary/20">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">Loop monedas</p>
            </div>
          </div>
        </Card>
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            return <Card key={challenge.id} className="p-6 hover:shadow-loopi transition-all duration-300 border-primary/20 bg-gradient-to-br from-card to-accent/5">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {badges.map(badge => <Card key={badge.id} className={`p-6 text-center transition-all duration-300 ${badge.unlocked ? "border-primary/20 bg-gradient-to-br from-card to-primary/10 hover:shadow-loopi" : "opacity-50 grayscale"}`}>
                <div className="text-5xl mb-3">{badge.icon}</div>
                <h3 className="font-bold mb-1">{badge.name}</h3>
                <p className="text-xs text-muted-foreground">{badge.description}</p>
                {badge.unlocked && <Badge className="mt-3 bg-gradient-loopi text-white">
                    Desbloqueada
                  </Badge>}
              </Card>)}
          </div>
        ...
        <Card className="p-8 bg-gradient-to-br from-accent/10 to-primary/10 border-accent/20">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {rewards.map(reward => <Card key={reward.id} className={`p-6 text-center ${reward.available ? "border-accent/50 bg-gradient-to-br from-card to-accent/10" : "opacity-70"}`}>
                ...
                <Button disabled={!reward.available} className={reward.available ? "bg-gradient-loopi hover:shadow-loopi w-full" : "w-full"}>
